...
- `media/` — turns a Jellyfin item into a prepared mpv play: `mod.rs` (`PreparedPlay`, `PlayRequest`, `prepare_play`, `select_media_source`), `streams.rs` (the `PlaybackInfo`/`MediaSource`/`MediaStream` serde models, the Jellyfin-index ↔ mpv-track-id maps, and `SubtitleId`), `subtitle.rs` (remembering the last hand-picked subtitle and re-finding it in the next episode by language and track name — stream indexes are per-file, so the index alone means nothing across episodes; `SubtitleMemory` is in-memory only and never persisted; a track picked in the mpv window counts as hand-picked too — `mpv.rs` observes `sid` and `runtime::playback::adopt_mpv_subtitle_track` maps it back to a Jellyfin stream index), `title.rs` (display titles).
- `ticks.rs` — Jellyfin position ticks (100 ns units) ↔ seconds. Used by `runtime/`, nothing to do with media.
- `mpv.rs` — `MpvSession`: spawns mpv with `--input-ipc-server`, `--force-window=yes`, `--idle=yes` (never `vo`/`hwdec`/`scale`/`glsl-shaders`, never `--no-config`), speaks JSON IPC over the Unix socket, exposes typed helpers (`loadfile`, `pause`, `seek_absolute`, …) and `MpvEvent`. `observe_subtitle_track` registers the one `observe_property` we use; `MpvEvent::SubtitleTrackChanged` deliberately carries no value, because property changes are handled long after they are emitted and `Runtime` re-reads `sid` instead (`settled_subtitle_track` is what separates a user's pick from mpv's own auto-selection).
