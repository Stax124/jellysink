...
fn tiles_fill_the_row_and_posters_pack_tighter_than_stills() {
    ...
    let posters = metrics(area, 2.0 / 3.0, FONT_SIZE, TARGET_ROWS);
    let stills = metrics(area, 16.0 / 9.0, FONT_SIZE, TARGET_ROWS);
...
fn a_tile_reserves_the_rows_under_its_cover_for_the_caption() {
    let grid = metrics(Rect::new(0, 0, 94, 40), 2.0 / 3.0, FONT_SIZE, TARGET_ROWS);
...
fn a_tall_area_spends_its_height_on_bigger_tiles_rather_than_more_rows() {
    let short = metrics(Rect::new(0, 0, 158, 20), 2.0 / 3.0, FONT_SIZE, TARGET_ROWS);
    let tall = metrics(Rect::new(0, 0, 158, 54), 2.0 / 3.0, FONT_SIZE, TARGET_ROWS);
...
fn a_short_area_keeps_its_tiles_rather_than_shrinking_them_for_a_second_row() {
    ...
    let aspect = 2.0 / 3.0;
    let grid = metrics(Rect::new(0, 0, 88, 18), aspect, FONT_SIZE, TARGET_ROWS);
...
fn a_wide_area_keeps_a_row_of_stills_rather_than_a_couple_of_huge_ones() {
    let grid = metrics(Rect::new(0, 0, 238, 54), 16.0 / 9.0, FONT_SIZE, TARGET_ROWS);
...
fn a_shelf_fits_a_row_of_tiles_into_half_a_body_however_short_it_is() {
    ...
    for height in [8, 10, 16, 23] {
        for aspect in [2.0 / 3.0, 16.0 / 9.0] {
            ...
            let shelf = metrics(area, aspect, FONT_SIZE, SHELF_ROWS);
...
fn a_shelf_spends_the_width_the_cover_cannot_use_on_more_tiles() {
    ...
    let shelf = metrics(area, 2.0 / 3.0, FONT_SIZE, SHELF_ROWS);
    assert!(
